...
// NormalizeFieldName 智能处理字段名，支持小驼峰和蛇形命名
// 如果字段名已经是蛇形命名（包含下划线），直接返回
// 如果字段名是小驼峰命名，转换为蛇形命名
func NormalizeFieldName(name string) string {
	// 如果字段名包含下划线，说明已经是蛇形命名，直接返回
...
// ConvertSort 转换排序参数为Order结构体数组
// sorts: 排序参数
// return: Order结构体数组, 错误信息
func ConvertSort(sorts *param.Sort) ([]*Order, error) {
	...
	for _, sort := range sorts.Fields {
		...
		if len(items) == 1 {
			// 使用智能字段名处理函数
			order.Property = NormalizeFieldName(items[0])
			// default asc
			order.Asc = true
